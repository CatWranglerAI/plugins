...
This invocation's verb: {{#claude}}`$verb`{{/claude}}{{#codex}}whatever the user asked for{{/codex}} — one of `list`, `add`, `remove`,
`connect`, or empty. Its slug: {{#claude}}`$slug`{{/claude}}{{#codex}}the project the user named{{/codex}} — empty when no project was
