#!/bin/sh
# Remote-login launcher (macOS/Linux only). Runtime selection is shared with hook startup.
...
case "$HOST" in
  claude|codex) ;;
  *)
    printf '%s\n' 'CatWrangler plugin: remote-login.sh requires an explicit claude or codex host.' >&2
    exit 2
    ;;
esac
...
exec "$NODE" "$DIR/remote-login.mjs" "$@"
