...
description: Restore CatWrangler MCP authentication after terminal OAuth expiry. Use only when CatWrangler reports terminal credential_expired or invalid_grant after normal reclaim cannot recover, or when the user asks to sign in to CatWrangler again from their phone.
---
{{/codex}}
...
{{#claude}}If this invocation's mode, `$mode`, is `remote`, or the human asks to sign in from their phone, go straight to "Signing in from a phone" below.{{/claude}}{{#codex}}If the human asks to sign in from their phone, go straight to "Signing in from a phone" below.{{/codex}}
...
1. Start the sign-in:
   ...
   ```
   {{#claude}}{{REMOTE_LOGIN}} start{{/claude}}{{#codex}}{{REMOTE_LOGIN_POSIX}} start{{/codex}}
   ```
{{#codex}}
   The launcher path is relative to **this skill's own directory**. Expand it against wherever you read this file from before running it. The launcher lives in the plugin root's `scripts/` directory.
{{/codex}}
...
2. Wait for the approval:
   ...
   ```
   {{#claude}}{{REMOTE_LOGIN}} wait{{/claude}}{{#codex}}{{REMOTE_LOGIN_POSIX}} wait{{/codex}}
   ```
